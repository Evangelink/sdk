...
public class DockerRegistryTests
{
    ...
    public DockerRegistryTests(MSTestContext testContext)
    {
    ...
    public async Task WriteToPrivateBasicRegistry()
    {
        ...
        try
        {
            ...
            new DotnetCommand(MSTestContext, $"dev-certs", "https", "--trust").Execute().Should().Pass();
            new DotnetCommand(MSTestContext, $"dev-certs", "https", "--export-path", registryCertFile, "--format", "PEM", "--no-password").Execute().Should().Pass();
            // start up an authenticated registry using that dev cert
            ContainerCli.RunCommand(MSTestContext,
                "-d", "--rm",
                "--name", "auth-registry",
                "-p", "5555:5000",
            ...
            ContainerCli.InspectCommand(MSTestContext, "auth-registry").Execute().Should().Pass();
            // login to that registry
            ContainerCli.LoginCommand(MSTestContext, "--username", "testuser", "--password", "testpassword", registryName).Execute().Should().Pass();
        ...
        {
            //stop the registry
            ContainerCli.StopCommand(MSTestContext, "auth-registry").WithWorkingDirectory(registryDir.FullName).Execute().Should().Pass();
        }
    }
}
