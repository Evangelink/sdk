...
namespace Microsoft.NET.Build.Tests
{
    public class GivenThatWeWantToBuildALibrary : SdkTest
    {
        ...
        [TestMethod]
        [DataRow(".NETStandard,Version=v1.0", new[] { "NETSTANDARD", "NETSTANDARD1_0", "NETSTANDARD1_0_OR_GREATER" })]
        [DataRow("netstandard1.3", new[] { "NETSTANDARD", "NETSTANDARD1_3", "NETSTANDARD1_0_OR_GREATER", "NETSTANDARD1_1_OR_GREATER", "NETSTANDARD1_2_OR_GREATER", "NETSTANDARD1_3_OR_GREATER" })]
...
class Program
{
    static void Main(string[] args)
    {
            ...
            var runCommand = new RunExeCommand(MSTestContext, Path.Combine(buildCommand.GetOutputDirectory(targetFramework).FullName, $"{testProj.Name}.exe"));
        ...
        public void It_defines_target_platform_defaults_correctly(string targetFramework, string propertyName, bool defaultsDefined)
        {
            ...
            var getValuesCommand = new GetValuesCommand(MSTestContext, Path.Combine(testAsset.TestRoot, testProject.Name), targetFramework, "TargetPlatformIdentifier");
            getValuesCommand
                .Execute()
                .Should()
                .Pass();
        ...
        public void It_defines_windows_version_default_correctly(string targetFramework)
        {
            ...
            var getValuesCommand = new GetValuesCommand(MSTestContext, Path.Combine(testAsset.TestRoot, testProject.Name), targetFramework, "TargetPlatformVersion");
            getValuesCommand
                .Execute()
                .Should()
                .Pass();
        ...
        private void TestInvalidTargetFramework(string testName, string targetFramework, bool useSolution, string expectedOutput)
        {
            ...
            if (useSolution)
            {
                ...
                restoreCommand = testAsset.GetRestoreCommand(MSTestContext, relativePathToSln);
                buildCommand = new BuildCommand(testAsset, relativePathToSln);
            }
            else
            {
                restoreCommand = testAsset.GetRestoreCommand(MSTestContext, testProject.Name);
            ...
            testProjectB.SourceFiles.Add("ProjB.cs", @"namespace ProjB
{
    public class ProjBClass
    {
        ...
        public void It_Has_Unescaped_PackageConflictPreferredPackages_Values()
        {
            ...
            var getValuesCommand = new GetValuesCommand(MSTestContext, Path.Combine(testAsset.TestRoot, "TestLibrary"), targetFramework, "PackageConflictPreferredPackages");
            getValuesCommand
                .Execute()
                .Should()
                .Pass();
