...
#pragma warning restore SA1308 // Variable names should not be prefixed
        ...
        public async Task CacheSearchNameMatchTest()
        {
            ...
            foreach (string? location in new[] { v1FileLocation, v2FileLocation })
            {
                ...
                Assert.ContainsSingle(searchResults);
                Assert.ContainsSingle(searchResults, result => result.Provider.Factory.DisplayName == "NuGet.org");
                ...
                (ITemplatePackageInfo _, IReadOnlyList<ITemplateInfo> packOneMatchedTemplates) = Assert.ContainsSingle(nugetSearchResults.SearchHits, pack => pack.PackageInfo.Name.Equals(s_packOneInfo.Name));
                (ITemplatePackageInfo _, IReadOnlyList<ITemplateInfo> packTwoMatchedTemplates) = Assert.ContainsSingle(nugetSearchResults.SearchHits, pack => pack.PackageInfo.Name.Equals(s_packTwoInfo.Name));
                Assert.ContainsSingle(packOneMatchedTemplates, t => string.Equals(t.Name, s_fooOneTemplate.Name));
                Assert.ContainsSingle(packTwoMatchedTemplates, t => string.Equals(t.Name, s_fooTwoTemplate.Name));
            }
        }
...
#pragma warning restore xUnit1004 // Test methods should not be skipped
        public async Task CacheSearchCliSymbolNameFilterTest()
        {
            ...
            foreach (string? location in new[] { v1FileLocation, v2FileLocation })
            {
                ...
                Assert.ContainsSingle(searchResults, result => result.Provider.Factory.DisplayName == "NuGet.org");
                SearchResult nugetSearchResults = searchResults.Single(result => result.Provider.Factory.DisplayName == "NuGet.org");
                Assert.ContainsSingle(nugetSearchResults.SearchHits);
...
#pragma warning restore xUnit1004 // Test methods should not be skipped
        ...
        public async Task CacheSearchLanguageFilterTest()
        {
            ...
            foreach (string? location in new[] { v1FileLocation, v2FileLocation })
            {
                ...
                SearchResult nugetSearchResults = searchResults.Single(result => result.Provider.Factory.DisplayName == "NuGet.org");
                Assert.ContainsSingle(nugetSearchResults.SearchHits);
                Assert.ContainsSingle(nugetSearchResults.SearchHits[0].MatchedTemplates);
